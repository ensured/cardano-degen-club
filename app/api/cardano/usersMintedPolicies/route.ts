...
import { CARDANO_NETWORK } from '@/components/NFTMinter'
...
export async function GET(request: Request) {
  ...
  if (!address || !keyHash || !blockfrostKey) {
    return NextResponse.json(
      { error: 'Address, keyHash, and blockfrostKey are required' },
      { status: 400 },
    )
  }
        ...
        for (const output of txDetails.output_amount) {
          if (output.unit !== 'lovelace') {
            ...
            if (!policies.has(policyId) && policyId.length === 56) {
              ...
              const scriptDetails = scriptDetailsResponse.ok
                ? await scriptDetailsResponse.json()
                : null
