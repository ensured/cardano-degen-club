...
export async function GET(request: NextRequest) {
  ...
  const searchParams = request.nextUrl ? new URL(request.url).searchParams : null;

  // For paginated fetching without rate-limiting
  if (searchParams && searchParams.get("nextPage")) {
    const nextPageUrl = `${searchParams.get("nextPage")}&app_key=${searchParams.get(
      "app_key"
    )}&_cont=${searchParams.get("_cont")}&type=${searchParams.get("type")}&app_id=${searchParams.get("app_id")}`;
  ...
  const rateLimitCheck = await checkRateLimit("fetchRecipes", ip, RECIPIES_FETCH_TIMEOUT_MS);
