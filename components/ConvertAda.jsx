...
const ConvertAda = ({ adaPrice = 0, btcPrice = 0 }) => {
  ...
  const handleAmountChange = (event) => {
    ...
    setAmount(isNaN(newAmount) ? '' : newAmount.length > 20 ? newAmount.slice(0, 20) : newAmount)
  }
  ...
  const convertedLovelaces = currency === 'ADA' && convertToLovelaces(amount, 1000000).toLocaleString()
    ...
    cryptoPrices.ADA !== 0 && (
      <div className="flex flex-col px-2 sm:px-1.5">
        <div className="flex flex-row items-center gap-2">
          <Switch id="currency-switch" checked={currency === 'ADA'} onCheckedChange={handleSwitchChange} />
          <Label htmlFor="currency-switch">
            <div className={'flex items-center justify-center gap-2'}> {currency}</div>
            {currency === 'ADA' ? (
              <div className={cn(`${currency === 'BTC' ? 'p-0' : 'px-2'}`)}>
                {cryptoPrices && <strong>${cryptoPrices.ADA ? cryptoPrices.ADA.toFixed(4) : 'N/A'}</strong>}
              </div>
            ) : (
              <div className="">
                {cryptoPrices && (
          ...
          {currency === 'ADA' ? <Icons.ada className="size-32" /> : <Icons.btc className="size-32" />}
          <Input
            type="number"
            min={'1'}
            value={amount}
        ...
        <div className="flex flex-col break-all">
          {currency === 'ADA' && cryptoPrices.BTC && (
            <div>BTC: {amount > 0 && <strong>{convertedBTC.toFixed(10)}</strong>}</div>
          )}

          {currency === 'BTC' && <div>ADA: {amount > 0 && <strong>{convertedADA}</strong>}</div>}

          <p>Sats: {amount > 0 && <strong>{convertedSats}</strong>}</p>

          {currency === 'ADA' && <p>Lovelaces: {amount > 0 && <strong>{convertedLovelaces}</strong>}</p>}
        </div>
      </div>
    )
  )
