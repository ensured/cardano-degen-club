...
import { getImagesBase64 } from '../app/actions'
			...
			>
				{Object.keys(favorites).length > 0 ? (
						...
						>
							{isLoadingPdfPreview ? (
								...
								<>
									...
									<div className="md:text-md line-clamp-2 items-center text-sm">Preview PDF</div>
								</>
							)}
						</Button>
						...
						>
							{isLoadingPdfDownload ? (
								...
								<>
									...
									<div className="md:text-md line-clamp-2 items-center text-sm">Download PDF</div>
								</>
							)}
						</Button>
					...
					<span className="text-md mt-4 flex flex-col items-center justify-center gap-2 text-center font-medium text-zinc-700 dark:text-zinc-300 md:text-lg">
						<span>
							You have <b>{Object.keys(favorites).length}</b> favorite recipes. Click the star button on a recipe to
							favorite it.
						</span>
					</span>
				)}
				...
				<div className="flex h-full flex-col gap-3">
					<div
						className={`animate-fade-in custom-scrollbar max-h-[64%] overflow-auto rounded-lg border bg-white/50 shadow-inner dark:bg-zinc-900/50 ${
...
const generatePDF = async (favorites, forDownload = false, onProgress) => {
	...
	try {
		...
		const urlsToFetch = imageUrls.filter((url) => !cachedImages.find((img) => img.url === url && img.base64))
		...
		const allImages = cachedImages.reduce(
			(acc, { url, base64 }) => {
		...
		for (const [link, { name, url }] of recipes) {
			...
			if (imageBase64) {
				doc.addImage(imageBase64, 'JPEG', xPos + CARD.padding, yPos + CARD.padding, CARD.imageSize, CARD.imageSize)
			}
