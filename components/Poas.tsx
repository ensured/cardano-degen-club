...
export default function Poas() {
			...
			if (!isValidImageFile(selectedFile)) {
				toast.error(
					<div className="flex flex-col gap-2">
						...
						<p className="text-sm text-muted-foreground">Supported formats: {formatSupportedExtensions()}</p>
					</div>,
		...
		if (selectedFile && !isValidImageFile(selectedFile)) {
			toast.error(
				<div className="flex flex-col gap-2">
					...
					<p className="text-sm text-muted-foreground">Supported formats: JPG, PNG, GIF, WebP, AVIF, SVG, TIFF, BMP</p>
	...
	const loadPolicies = async () => {
		if (!api || !blockfrostKey) {
			toast.error('Please connect wallet and enter Blockfrost key first', { position: 'bottom-center' })
			return
		}
		...
		try {
			...
			const lucid = await Lucid(
				new Blockfrost(`https://cardano-${CARDANO_NETWORK.toLowerCase()}.blockfrost.io/api/v0`, blockfrostKey),
				CARDANO_NETWORK,
			)
			...
			if (Number(balance) < 1000000) {
				toast.error('Insufficient balance, please add funds to your wallet', { position: 'bottom-center' })
				return
			}
		...
		if (!isImage) {
			toast.error(
				<div className="flex flex-col gap-2">
					<p>Invalid file type</p>
					<p className="text-sm text-muted-foreground">Supported formats: JPG, PNG, GIF, WebP, AVIF, SVG, TIFF, BMP</p>
					<p className="text-xs text-muted-foreground">Selected file type: {file.mime_type || 'unknown'}</p>
				</div>,
				{ position: 'bottom-center' },
			)
			return
		...
		<main className="m-auto flex w-full max-w-3xl flex-col items-center justify-center gap-4 p-4">
			...
			<TooltipProvider delayDuration={50}>
				<div className="mb-4 flex w-full justify-between px-2">
					{[
						...
						{ step: 2, tooltip: 'Upload your NFT asset to IPFS or select a file from Pinata', title: 'IPFS/Storage' },
			...
			>
				<CollapsibleTrigger className="flex w-full items-center justify-between p-6 hover:bg-secondary/10">
					<h2 className="text-xl font-semibold">Step 1: Configure API Keys</h2>
			...
			{blockfrostKey && pinataJWT && (
				...
				>
					<CollapsibleTrigger className="flex w-full items-center justify-between p-6 hover:bg-secondary/10">
						<h2 className="text-lg font-semibold">Step 2: Upload/Choose Asset</h2>
						<div className="flex items-center gap-2">
					...
					<CollapsibleContent className="px-6 pb-6">
						<div className="flex flex-col gap-4">
							<div className="flex gap-4">
								...
								/>
								<Button3D disabled={!isStepComplete(1) || uploading || !file} onClick={() => uploadFile(file)}>
							...
							{url && (
								<div className="mt-2 flex items-center gap-4 rounded-lg bg-secondary/20 p-4">
									...
									<div className="flex flex-col gap-1">
										<div className="text-sm text-muted-foreground">IPFS Hash: {url.slice(0, 20)}...</div>
			...
			{url && (
				...
				>
					<CollapsibleTrigger className="flex w-full items-center justify-between p-6 hover:bg-secondary/10">
						<h2 className="text-lg font-semibold">Step 3: Select Policy ID</h2>
						<div className="flex items-center gap-2">
						...
						<div className="col-span-1 flex flex-col gap-4">
							...
							<DropdownMenu>
								...
								<DropdownMenuContent className="max-h-96 w-[var(--radix-dropdown-menu-trigger-width)] overflow-y-auto">
									...
									{policyIds.some((p) => p.isGenerated) && policyIds.some((p) => !p.isGenerated) && (
										<DropdownMenuSeparator />
									)}
							...
							.map((file) => (
								...
								>
									<div className="space-y-2">
										...
										<p className="text-xs text-muted-foreground">{new Date(file.date_pinned).toLocaleDateString()}</p>
									</div>
								</div>
							))}
					</div>
