...
export default function SlotConverter({
  network,
  defaultSlot,
  selectedDateTime,
}: SlotConverterProps) {
  ...
  useEffect(() => {
    if (selectedDateTime && currentEpochData) {
      ...
      const slotNumber = Math.floor(
        timestamp - currentEpochData.start_time + currentEpochData.epoch * SLOTS_PER_EPOCH,
      )
  ...
  const calculateDate = (slotNumber: number): Date | null => {
    ...
    try {
      ...
      const timestamp =
        currentEpochData.start_time + (epochsDiff * SLOTS_PER_EPOCH + slotsIntoEpoch) * SLOT_LENGTH
          ...
          <div className="rounded-md bg-secondary/20 p-3">
            {date ? (
              ...
              <p className="text-sm text-muted-foreground">
                {currentEpochData
                  ? 'Enter a valid slot number to see the date'
                  : 'Loading epoch data...'}
              </p>
            )}
          </div>
